...
class State(HydraHeadApp):
    ...
    def load_graphs(self):
        ...
        with col1:
            ...
            #print("does this print?") #yes it does on the output screen of the cmd line
            print(year_where)
            #print(where)
    ...
    def temperature_Query(self, temperature):
        output = [int(i) for i in temperature.split() if i.isdigit()]
        siteTemp = """SELECT * FROM "J.POULOS".Accident 
                        WHERE ROWNUM < 20 AND temperature
                        BETWEEN :out0 AND :out1"""
        self.cursor.execute(siteTemp, out0 = output[0])
        self.cursor.execute(siteTemp, out1 = output[1])
        
          
        #temperature holds the value range and needs to be converted
        # to just integer values
        #maybe split into two separate variables{start, end}
        # to use in the query BETWEEN start AND end    
        #values in DB are float so doing a range of 35-69
        #and starting the next range at 70 loses 69.1-69.9

#Time will require date as well!
#
#15-FEB-16 01.03.46.000000000 PM 15-FEB-16 07.03.46.000000000 PM
#
